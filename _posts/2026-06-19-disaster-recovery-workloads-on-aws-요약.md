...
1. AWS와 고객 사이의 resiliency shared responsibility model을 설명한다.

1. “재해”를 자연재해, 기술 장애, 인간 행위로 나누고, 그 지리적 범위를 local, regional, country-wide, continental, global 수준으로 판단하게 한다.

1. DR 계획은 독립 문서가 아니라 Business Continuity Plan(BCP)의 일부여야 한다고 강조한다.

1. Business Impact Analysis와 Risk Assessment를 통해 RTO와 RPO를 정한다.

1. AWS Cloud에서 DR이 기존 온프레미스 DR과 어떻게 다른지 설명한다.

1. 네 가지 DR 전략을 비용과 복잡도, RTO/RPO 수준에 따라 비교한다.

1. 마지막으로 detection과 testing 없이는 DR 전략이 실제로 작동한다고 볼 수 없다고 정리한다.
...
- RTO(Recovery Time Objective): 서비스 중단 후 복구까지 허용 가능한 최대 시간

- RPO(Recovery Point Objective): 마지막 복구 지점 이후 허용 가능한 최대 데이터 손실 시간

백서는 RTO/RPO가 기술팀의 희망사항이 아니라 비즈니스 영향 분석과 리스크 평가의 결과여야 한다고 본다. 또한 RTO/RPO를 낮출수록 비용과 복잡도가 증가한다.
...
리뷰 관점에서 이 부분은 매우 중요하다. AWS managed service를 쓴다고 DR 책임이 사라지는 것이 아니라, DR 설계의 추상화 수준이 올라갈 뿐이다. 예를 들어 S3는 매우 내구성이 높지만, 잘못된 삭제나 악의적 변경으로부터 보호하려면 versioning, replication, object lock, point-in-time 성격의 백업 전략을 별도로 고려해야 한다.
...
아래 이미지는 AWS 문서에서 제공되는 원본 이미지 URL을 그대로 사용했다.

### Figure 1 - Resiliency Objectives

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/resiliency-objectives.png

### Availability calculation - time based

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/availability-calculation-time-based.png

### Availability calculation - successful/failed requests

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/availability-calculation-successful-failed-requests.png

### Figure 3 - Recovery objectives

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/recovery-objectives.png

### Figure 4 - Recovery time objective

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/recovery-time-objective.png

### Figure 5 - Recovery point objective

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/recovery-point-objective.png

### Figure 2 - Shared responsibility of resiliency

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/shared-responsibility-of-resiliency.png
...
- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/backup-restore-architecture.png

### Figure 8 - Restoring and testing backups

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/restore-test-backups.png

### Figure 9 - Pilot light architecture

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/pilot-light-architecture.png

### Figure 10 - AWS Elastic Disaster Recovery architecture

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/disaster-recovery-architecture.png

### Figure 11 - Warm standby architecture

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/warm-standby-architecture.png

### Figure 12 - Multi-site active/active architecture

!/assets/image_2ea7f913-9688-410a-8ad3-f861f85a3cf3.png

- URL: https://docs.aws.amazon.com/images/whitepapers/latest/disaster-recovery-workloads-on-aws/images/multi-site-active-active-architecture.png
...
1. Active/Active의 데이터 일관성 문제는 더 깊은 설계가 필요하다.
...
1. 보안 재해에 대한 runbook은 더 구체화할 필요가 있다.
...
1. 네 가지 DR 전략 중 하나를 선택한다.

1. IaC, backup, replication, routing, 계정/Region 분리를 설계한다.

1. control plane 의존성을 줄인다.

1. 정기 DR drill과 drift detection을 운영 체계에 넣는다.
